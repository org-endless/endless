...
public class AsyncTaskMangerService {
    ...
    public AsyncTaskMangerService(MongoOperations mongoOperations, AsyncTaskMangerRepositoryBase asyncTaskMangerJpaRepository) {
    ...
    public void create(String id, AsyncTaskManager.Status status) {
        ...
        if (asyncTaskMangerJpaRepository.existsById(id)) {
            ...
            var result = mongoOperations.update(AsyncTaskManager.class)
                    .matching(Query.query(Criteria.where("id").is(id)
                            .and("statue").is(AsyncTaskManager.Status.exit)))
            ...
            mongoOperations.update(AsyncTaskManager.class)
                    .matching(Query.query(Criteria.where("id").is(id)))
                    .apply(Update.update("asyncTaskId", id.split("_")[1])
