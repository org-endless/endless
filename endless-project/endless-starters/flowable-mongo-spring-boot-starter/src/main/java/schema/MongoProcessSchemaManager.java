...
import persistence.MongoDbSession;
import persistence.MongoDbSessionFactory;
import persistence.manager.*;

import java.util.*;

/**
 * Note that the term 'schema' is not the same as for a relational database. MongoDB doesn't have a
 * schema that forces the data in a strict structure.
 *
 * <p>What this class does is: - Making sure all collections needed by the Flowable Process engine
 * are created. - Making sure the necessary indices have been created on those collections.
 *
 * <p>Note we're not using the schema validation feature of MongoDB (see
 * <a href="https://docs.mongodb.com/manual/core/schema-validation/">...</a>). This would only give us a json schema
 * validation check on insert/update but none of the structural schema benefits of the relational
 * counterpart.
 *
 * @author Joram Barrez
 */
...
public class MongoProcessSchemaManager implements SchemaManager {
    ...
    public void schemaCreate() {
        ...
        if (currentSchemaVersion == null) {
            ...
            throw new FlowableException(
                    "Cannot create collections and indices: previous version already exists");
        }
    }

    protected void initializeDefaultCollectionsAndIndices(
            MongoDbProcessEngineConfiguration engineConfiguration) {
    ...
    protected void initDefaultCollections(MongoDbProcessEngineConfiguration engineConfiguration) {
        // Collections can't be created in a transaction (see
        // https://docs.mongodb.com/manual/core/transactions/)
        Set<String> collections = new HashSet<>();
        MongoIterable<String> existingCollections =
                engineConfiguration.getMongoDatabase().listCollectionNames();
    ...
    protected void initDefaultIndices(MongoDbProcessEngineConfiguration engineConfiguration) {
        ...
        mongoDatabase
                .getCollection(MongoDbResourceDataManager.COLLECTION_BYTE_ARRAY)
                .createIndex(new Document("deploymentId", 1));

        mongoDatabase
                .getCollection(MongoDbProcessDefinitionDataManager.COLLECTION_PROCESS_DEFINITIONS)
                .createIndex(new Document("deploymentId", 1));

        mongoDatabase
                .getCollection(MongoDbEventSubscriptionDataManager.COLLECTION_EVENT_SUBSCRIPTION)
                .createIndex(new Document("executionId", 1));
        mongoDatabase
                .getCollection(MongoDbEventSubscriptionDataManager.COLLECTION_EVENT_SUBSCRIPTION)
                .createIndex(new Document("processDefinitionId", 1));
        mongoDatabase
                .getCollection(MongoDbEventSubscriptionDataManager.COLLECTION_EVENT_SUBSCRIPTION)
                .createIndex(new Document("processInstanceId", 1));
        mongoDatabase
                .getCollection(MongoDbEventSubscriptionDataManager.COLLECTION_EVENT_SUBSCRIPTION)
                .createIndex(new Document("configuration", 1));

        mongoDatabase
                .getCollection(MongoDbExecutionDataManager.COLLECTION_EXECUTIONS)
                .createIndex(new Document("businessKey", 1));
        mongoDatabase
                .getCollection(MongoDbExecutionDataManager.COLLECTION_EXECUTIONS)
                .createIndex(new Document("processInstanceId", 1));
        mongoDatabase
        ...
        mongoDatabase
                .getCollection(MongoDbExecutionDataManager.COLLECTION_EXECUTIONS)
                .createIndex(new Document("parentId", 1));
        mongoDatabase
                .getCollection(MongoDbExecutionDataManager.COLLECTION_EXECUTIONS)
                .createIndex(new Document("superExecutionId", 1));
        mongoDatabase
                .getCollection(MongoDbExecutionDataManager.COLLECTION_EXECUTIONS)
                .createIndex(new Document("rootProcessInstanceId", 1));

        mongoDatabase
                .getCollection(MongoDbTaskDataManager.COLLECTION_TASKS)
                .createIndex(new Document("executionId", 1));
        mongoDatabase
                .getCollection(MongoDbTaskDataManager.COLLECTION_TASKS)
                .createIndex(new Document("processInstanceId", 1));
        ...
        mongoDatabase
                .getCollection(MongoDbTaskDataManager.COLLECTION_TASKS)
                .createIndex(new Document("createTime", 1));
        mongoDatabase
                .getCollection(MongoDbTaskDataManager.COLLECTION_TASKS)
                .createIndex(new Document().append("scopeId", 1).append("scopeType", 1));
        mongoDatabase
                .getCollection(MongoDbTaskDataManager.COLLECTION_TASKS)
                .createIndex(new Document().append("subScopeId", 1).append("scopeType", 1));
        mongoDatabase
                .getCollection(MongoDbTaskDataManager.COLLECTION_TASKS)
                .createIndex(new Document().append("scopeDefinitionId", 1).append("scopeType", 1));

        mongoDatabase
                .getCollection(MongoDbVariableInstanceDataManager.COLLECTION_VARIABLES)
                .createIndex(new Document("executionId", 1));
        mongoDatabase
                .getCollection(MongoDbVariableInstanceDataManager.COLLECTION_VARIABLES)
                .createIndex(new Document("processInstanceId", 1));
        ...
        mongoDatabase
                .getCollection(MongoDbVariableInstanceDataManager.COLLECTION_VARIABLES)
                .createIndex(new Document("taskId", 1));
        mongoDatabase
                .getCollection(MongoDbVariableInstanceDataManager.COLLECTION_VARIABLES)
                .createIndex(new Document().append("scopeId", 1).append("scopeType", 1));
        mongoDatabase
        ...
        mongoDatabase
                .getCollection(MongoDbIdentityLinkDataManager.COLLECTION_IDENTITY_LINKS)
                .createIndex(new Document("processInstanceId", 1));
        mongoDatabase
                .getCollection(MongoDbIdentityLinkDataManager.COLLECTION_IDENTITY_LINKS)
                .createIndex(new Document("processDefinitionId", 1));
        ...
        mongoDatabase
                .getCollection(MongoDbIdentityLinkDataManager.COLLECTION_IDENTITY_LINKS)
                .createIndex(new Document("userId", 1));
        mongoDatabase
                .getCollection(MongoDbIdentityLinkDataManager.COLLECTION_IDENTITY_LINKS)
                .createIndex(new Document("groupId", 1));
        mongoDatabase
                .getCollection(MongoDbIdentityLinkDataManager.COLLECTION_IDENTITY_LINKS)
                .createIndex(new Document().append("scopeId", 1).append("scopeType", 1));
        mongoDatabase
        ...
        mongoDatabase
                .getCollection(MongoDbJobDataManager.COLLECTION_JOBS)
                .createIndex(new Document("executionId", 1));
        mongoDatabase
                .getCollection(MongoDbJobDataManager.COLLECTION_JOBS)
                .createIndex(new Document("processInstanceId", 1));
        ...
        mongoDatabase
                .getCollection(MongoDbTimerJobDataManager.COLLECTION_TIMER_JOBS)
                .createIndex(new Document("executionId", 1));
        mongoDatabase
                .getCollection(MongoDbTimerJobDataManager.COLLECTION_TIMER_JOBS)
                .createIndex(new Document("processInstanceId", 1));
        ...
        mongoDatabase
                .getCollection(MongoDbJobByteArrayDataManager.COLLECTION_JOB_BYTE_ARRAY)
                .createIndex(new Document("deploymentId", 1));

        mongoDatabase
                .getCollection(
                        MongoDbHistoricActivityInstanceDataManager.COLLECTION_HISTORIC_ACTIVITY_INSTANCES)
                .createIndex(new Document("startTime", 1));
        mongoDatabase
                .getCollection(
                        MongoDbHistoricActivityInstanceDataManager.COLLECTION_HISTORIC_ACTIVITY_INSTANCES)
                .createIndex(new Document("endTime", 1));
        mongoDatabase
                .getCollection(
                        MongoDbHistoricActivityInstanceDataManager.COLLECTION_HISTORIC_ACTIVITY_INSTANCES)
                .createIndex(new Document("processInstanceId", 1));
        ...
        mongoDatabase
                .getCollection(MongoDbHistoricIdentityLinkDataManager.COLLECTION_HISTORIC_IDENTITY_LINKS)
                .createIndex(new Document("userId", 1));
        mongoDatabase
                .getCollection(MongoDbHistoricIdentityLinkDataManager.COLLECTION_HISTORIC_IDENTITY_LINKS)
                .createIndex(new Document("groupId", 1));
        ...
        mongoDatabase
                .getCollection(MongoDbHistoricTaskInstanceDataManager.COLLECTION_HISTORIC_TASK_INSTANCES)
                .createIndex(new Document("processInstanceId", 1));
        mongoDatabase
                .getCollection(MongoDbHistoricTaskInstanceDataManager.COLLECTION_HISTORIC_TASK_INSTANCES)
                .createIndex(new Document("processDefinitionId", 1));
        ...
        mongoDatabase
                ...
                .createIndex(new Document().append("name", 1).append("typeName", 1));
        mongoDatabase
                .getCollection(
                        MongoDbHistoricVariableInstanceDataManager.COLLECTION_HISTORIC_VARIABLE_INSTANCES)
                .createIndex(new Document().append("scopeId", 1).append("scopeType", 1));
    ...
    protected boolean acquireLock(MongoDbProcessEngineConfiguration engineConfiguration) {
        try {
            // Try to write a random ID to the lock document. There is a unique index on the name, so
            // there can be only one.
            // If the duplicate key exception is thrown, another engine has acquired the lock first.
            MongoCollection<Document> propertiesCollection = getPropertiesCollection(engineConfiguration);
            propertiesCollection.insertOne(
                    new Document().append("name", "lock").append("value", randomId));

            // Even if the write was successful, double-check the value of the written lock
            FindIterable<Document> lockDocumentIterable =
                    propertiesCollection.find(new Document().append("name", "lock"));
    ...
    protected void releaseLock(MongoDbProcessEngineConfiguration engineConfiguration) {
        ...
        if (propertiesCollection != null) {
            propertiesCollection.findOneAndDelete(
                    new Document().append("name", "lock").append("value", randomId));
        }
    }

    protected MongoCollection<Document> getPropertiesCollection(
            MongoDbProcessEngineConfiguration engineConfiguration) {
        if (!propertiesCollectionExists(engineConfiguration.getMongoDatabase())) {
            ...
            try {
                ...
                propertiesCollection =
                        engineConfiguration.getMongoDatabase().getCollection(COLLECTION_PROPERTIES);
                propertiesCollection.createIndex(
                        new Document("name", 1),
                        new IndexOptions().unique(true).name("flw_schema_version_idx"));
            } catch (Exception e) {
                // ignore
            }
    ...
    public void schemaDrop() {
        log.info("Dropping all MongoDB collections in the database");
        ...
        if (currentSchemaVersion == null) {
            ...
            try {
                ...
                if (!FlowableVersions.CURRENT_VERSION.equals(currentSchemaVersion)) {
                    // TODO in future release (if needed): loop from current version to latest version and
                    // apply updates programmatically
    ...
    protected String getSchemaVersionValue(MongoDatabase mongoDatabase) {
        MongoCollection<Document> propertiesCollection =
                mongoDatabase.getCollection(COLLECTION_PROPERTIES);
        FindIterable<Document> schemaVersionDocument =
                propertiesCollection.find(Filters.eq("name", SCHEMA_VERSION_PROPERTY));
    ...
    protected Collection<String> getAllCollectionNames() {
        MongoDbSessionFactory mongoDbSessionFactory =
                (MongoDbSessionFactory)
                        getEngineConfiguration().getSessionFactories().get(MongoDbSession.class);
        Set<String> collectionNames = new HashSet<>(mongoDbSessionFactory.getCollectionNames());
        collectionNames.add(COLLECTION_PROPERTIES);
        return collectionNames;
    }

    protected MongoDbProcessEngineConfiguration getEngineConfiguration() {
        return (MongoDbProcessEngineConfiguration) CommandContextUtil.getProcessEngineConfiguration();
    }
}
