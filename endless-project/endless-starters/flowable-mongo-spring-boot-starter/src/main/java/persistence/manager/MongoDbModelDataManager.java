...
import persistence.entity.MongoDbModelEntityImpl;
 ...
 * @author Joram Barrez
 */
public class MongoDbModelDataManager extends AbstractMongoDbDataManager<ModelEntity>
    ...
    public void insert(ModelEntity modelEntity) {
        MongoDbModelEntityImpl latestModel =
                (MongoDbModelEntityImpl) findLatestModel(modelEntity.getKey());
        if (latestModel == null) {
    ...
    protected ModelEntity findLatestModel(String key) {
        return getMongoDbSession()
                .findOne(COLLECTION_MODELS, Filters.eq("key", key), Sorts.descending("version"), 1);
    }
