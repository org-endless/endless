 ...
 * @author Joram Barrez
 */
public class MongoDbHistoricActivityInstanceDataManager
        extends AbstractMongoDbDataManager<HistoricActivityInstanceEntity>
    ...
    public MongoDbHistoricActivityInstanceDataManager(
    ...
    @Override
    public List<HistoricActivityInstanceEntity>
    findHistoricActivityInstancesByExecutionIdAndActivityId(
            String executionId, String activityId) {
    ...
    public void deleteHistoricActivityInstancesByProcessInstanceId(String historicProcessInstanceId) {
        getMongoDbSession()
                .bulkDelete(
                        COLLECTION_HISTORIC_ACTIVITY_INSTANCES,
                        Filters.eq("processInstanceId", historicProcessInstanceId));
    }
    ...
    public long findHistoricActivityInstanceCountByQueryCriteria(
        ...
        return getMongoDbSession()
                .count(COLLECTION_HISTORIC_ACTIVITY_INSTANCES, createFilter(historicActivityInstanceQuery));
    }
    ...
    public List<HistoricActivityInstance> findHistoricActivityInstancesByQueryCriteria(
            HistoricActivityInstanceQueryImpl historicActivityInstanceQuery) {
        return getMongoDbSession()
                .find(COLLECTION_HISTORIC_ACTIVITY_INSTANCES, createFilter(historicActivityInstanceQuery));
    }
    ...
    @Override
    public void deleteHistoricActivityInstances(
            HistoricActivityInstanceQueryImpl historicActivityInstanceQuery) {
        throw new UnsupportedOperationException();
    }
    ...
    protected Bson createFilter(HistoricActivityInstanceQueryImpl activityInstanceQuery) {
        ...
        if (activityInstanceQuery.getProcessDefinitionId() != null) {
            filters.add(
                    Filters.eq("processDefinitionId", activityInstanceQuery.getProcessDefinitionId()));
        }
        ...
        if (activityInstanceQuery.getDeleteReasonLike() != null) {
            filters.add(
                    Filters.regex(
                            "deleteReason", activityInstanceQuery.getDeleteReasonLike().replace("%", ".*")));
        }
        ...
        if (activityInstanceQuery.getTenantIdLike() != null) {
            filters.add(
                    Filters.regex("tenantId", activityInstanceQuery.getTenantIdLike().replace("%", ".*")));
        }
