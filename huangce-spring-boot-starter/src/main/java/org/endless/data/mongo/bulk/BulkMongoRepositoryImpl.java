...
import org.endless.com.utiliy.list.ListSplitter;
...
public class BulkMongoRepositoryImpl implements BulkMongoRepository {
    ...
    private final BulkMongoUpsertTask asyncTask;

    public BulkMongoRepositoryImpl(
            MongoOperations mongoOperations,
            @Qualifier("bulkMongoUpsertTask") BulkMongoUpsertTask asyncTask) {
    ...
    public void upsert(List<Pair<Query, Update>> pairs, Integer pageSize, Class<?> entityClass) {
        ...
        pages.forEach(
                page -> {
                    asyncTask.run(page, entityClass);
                });
    }
}
